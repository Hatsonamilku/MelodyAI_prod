# services/discord_adapter.py - COMPLETE FIXED VERSION
...
class DiscordMelodyAdapter:
    ...
    async def test_channel_communication(self, channel: discord.TextChannel):
        ...
        try:
            ...
            print(f"❌ Fact extraction test failed (import): {e}")
            return False
        except Exception as e:
        ...
        try:
            ...
            print(f"❌ AI test failed (import): {e}")
            return False
        except Exception as e:
    ...
    async def process_discord_message(self, message: discord.Message, ai_provider=None, respond: bool = True) -> Optional[str]:
        ...
        try:
            ...
            logger.error(f"❌ Import error in process_discord_message: {e}")
            return "My systems are currently being upgraded! Try again in a moment! 🔧"
        except Exception as e:
            logger.error(f"❌ Error in process_discord_message: {e}")
    ...
    async def _summarize_conversation(self, channel_id: int, ai_provider=None):
        ...
        try:
            ...
            logger.error(f"❌ Import error in conversation summarization: {e}")

    async def handle_mention(self, message: discord.Message, ai_provider=None) -> str:
        """Handle when the bot is mentioned directly"""
        user_id = str(message.author.id)
    ...
    def get_user_emotional_state(self, user_id: str) -> Dict[str, any]:
        """Get user emotional state - placeholder for emotional core integration"""
        try:
            from brain.personality.emotional_core import emotional_core
            return emotional_core.get_emotional_state(user_id)
        except ImportError:
            return {"score": 50, "mood": "neutral"}

    def get_user_facts(self, user_id: str) -> str:
        """Get user facts - placeholder for permanent facts integration"""
        try:
            from brain.memory_systems.permanent_facts import permanent_facts
