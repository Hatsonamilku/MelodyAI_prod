# melody_ai_v2/brain/core_intelligence/intelligence_orchestrator.py - FIXED VERSION
import asyncio
import logging
import random
...
class IntelligenceOrchestrator:
    ...
    def __init__(self):
        # Initialize with None, will load via lazy imports
        self._emotional_core = None
        self._permanent_facts = None
        self._semantic_memory = None
        logger.info("🧠 Intelligence Orchestrator initialized!")

    @property
    def emotional_core(self):
        """Lazy load emotional core"""
        if self._emotional_core is None:
            from brain.personality.emotional_core import emotional_core
            self._emotional_core = emotional_core
        return self._emotional_core

    @property
    def permanent_facts(self):
        """Lazy load permanent facts"""
        if self._permanent_facts is None:
            from brain.memory_systems.permanent_facts import permanent_facts
            self._permanent_facts = permanent_facts
        return self._permanent_facts
