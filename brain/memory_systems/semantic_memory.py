# melody_ai_v2/🧠 brain/memory_systems/semantic_memory.py
import sqlite3
import numpy as np
...
class SemanticMemorySystem:
    def __init__(self, db_path='melody_memory.db'):
        ...
        try:
            ...
            self.memory_map = {}  # Maps FAISS index to conversation data
    ...
    def store_conversation(self, user_id: str, user_message: str, bot_response: str, importance: float = 1.0):
        ...
        cursor = self.conn.cursor()
        cursor.execute('''
            SELECT COALESCE(MAX(memory_id), 0) + 1 
        ...
        embedding = self.model.encode([conversation_text])[0]
    ...
    def search_relevant_memories(self, user_id: str, query: str, top_k: int = 5) -> List[Dict[str, Any]]:
    ...
    def get_conversation_context(self, user_id: str, current_message: str) -> str:
        """Get relevant conversation context for AI response"""
        relevant_memories = self.search_relevant_memories(user_id, current_message, top_k=3)
